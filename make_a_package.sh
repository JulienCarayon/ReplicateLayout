...
wxformbuilder=/Applications/wxFormBuilder.app/Contents/MacOS/wxFormBuilder
...
sed -i '' "s/VERSION/$version/g" metadata.json
...
version=`cat version.txt`
sed -i '' "s/VERSION/$version/g" metadata.json
zipsha=`shasum -a 256 ReplicateLayout-$version-pcm.zip | xargs | cut -d' ' -f1`
sed -i '' "s/SHA256/$zipsha/g" metadata.json
unzipsize=`unzip -l ReplicateLayout-$version-pcm.zip | tail -1 | xargs | cut -d' ' -f1`
sed -i '' "s/INSTALL_SIZE/$unzipsize/g" metadata.json
dlsize=`ls -al ReplicateLayout-$version-pcm.zip | tail -1 | xargs | cut -d' ' -f5`
sed -i '' "s/DOWNLOAD_SIZE/$dlsize/g" metadata.json
